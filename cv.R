...
get_folds <- function(allData, cvSetting, k) {
  ...
  if (cvSetting == 'S2') {
    ...
    for (i in 1:k) {
      ## if it is leave-target-out CV, then testCompounds = i
...
kfoldcv <- function(allData, 
  ...
  k = length(folds)                            ## just in case 'k' was modified
  predictions <- list()
  for (i in 1:k) {
    ## get test set
    testSet <- folds[[i]]
    ...
    if (cvSetting == 'S4') {    ## if it is the special case of S4...
      ## exclude records containing compounds or targets that exist in test set
      trainingSet <- 
        allData %>% 
    ...
    predictions[[i]] <- predFun(trainingSet, testSet, predFunParams)
  }
  ...
  if (poolingCV) {    ## if pooling CV is to be used...
    ...
    evalResults <- evaluate(predictions, labels, evalMetrics)
    ...
    for (i in 1:k) {
      evalResults[i,] <- evaluate(predictions, labels, evalMetrics)
    }
